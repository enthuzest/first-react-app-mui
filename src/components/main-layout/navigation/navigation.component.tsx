...
import {
  AppBar,
  Box, Button, Dialog, Hidden, IconButton,
  Slide,
  Toolbar,
  Typography,
} from '@mui/material';
...
export const Navigation: React.FC<NavigationProps> = ({ isSmall }) => {
  ...
  const OnClosedHandler = () => setOpen(false);

  const mappedItems = (
    navigationItems.map(({ to, text }) => {
      return (
        <AnchorLink key={to} href={`#${to}`} offset={isSmall ? '56px' : '64px'} className="all_unset">
          <Button color="inherit" size="large" fullWidth={isSmall} onClick={OnClosedHandler}>
            {text}
          </Button>
        </AnchorLink>
      );
        ...
        >
          <AppBar position="static" sx={{ background: 'white', color: 'text.primary' }}>
            <Toolbar>
              ...
              <IconButton color="inherit" onClick={OnClosedHandler}>
                <Close />
              </IconButton>
            </Toolbar>
          </AppBar>
